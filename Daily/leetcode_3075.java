...
public class leetcode_3075 {
    ...
    public static long maximumHappinessSum(int[] happiness, int k) {
        ...
        long sum = 0;
        int dec = 0;              // how many children already picked

        for (int i = happiness.length - 1; i >= 0 && k > 0; i--) {

            long value = happiness[i] - dec;

            if (value < 0) value = 0;   // cannot go below 0
    ...
    //         if(happiness[i]>=happy && k>0){
    //             k--;
    //             happiness[i]=happiness[i]-happy;
    //             sum=sum+happiness[i];
